...
def draw_panels(panels):
    lines = []
    min_x, max_x, min_y, max_y = get_bounds(panels)
    for y in range(max_y, min_y - 1, -1):
        lines.append([panels.get((x, y), 0) for x in range(min_x, max_x + 1)])
    for line in lines:
        print("".join("X" if l else " " for l in line))


def get_bounds(panels):
    xs, ys = zip(*panels)
