...
def main():
    ...
    time_taken = calc_time_to_oxygenate(map)
    return time_taken


def neigbouring_tiles(map, tile):
    neighbours = (new_position(tile, move) for move in range(1, 5))
    return [t for t in neighbours if map[t] in (1, 2)]


def calc_time_to_oxygenate(map):
    time_taken = 0
    while True:
        tiles_left = sum(1 if v == 1 else 0 for v in map.values())
        if not tiles_left:
            break
        for tile, value in list(map.items()):
            if value == 2:
                # spread to neighbours
                for neighbour in neigbouring_tiles(map, tile):
                    map[neighbour] = 2
